...
| Follow-up A | **Separate ZPE vs harmonic frequency scaling factors**. The Truhlar database we use stores `zpe_fac` and `harm_fac` as distinct values (Alecu et al., JCTC 2010); GoodVibes currently applies `harm_fac` to both ZPE and the partition-function frequencies. The two factors typically differ by ~1% — small per mode but ~0.1–0.3 kcal/mol on a typical organic. Plan: read both from the DB, route `zpe_fac` into `calc_zeropoint_energy` and `harm_fac` into `calc_vibrational_energy` / `calc_rrho_entropy`. Add `--strict-scaling` to opt in (preserves back-compat for cited papers); make it the default in v5.0 with a migration note. ~50 lines + golden-value updates. | Pending |
