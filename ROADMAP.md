...
| # | Item | Status |
| --- | --- | --- |
| 10 | **First-class structured outputs**: promote `--json` and `--csv` to documented stable schemas (`schema_version: "1.0"`). Parquet via pandas behind extras. Define `goodvibes.schema`. **Subsumes `--cache-save` / `--cache-read`** under one schema with `--export` / `--import` modes. | In progress (phase 1: schema 1.0 + `goodvibes.schema` validator + `--parquet`/`to_parquet` shipped; phase 2: `--import`/`--export` flags + cache deprecation pending) |
