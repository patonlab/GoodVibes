...
| 14 | **Visualization** (stretch): new `goodvibes/plot.py` with `plot_selectivity_strip` (per-species ΔG scatter + Boltzmann-mean / lowest overlays — the headline) and `plot_pes` (clean PES profile from `PESResult`). New `--strip-plot PATH` CLI flag. `[plot]` extras add matplotlib. Stubs for `plot_boltzmann_histogram` and `plot_temperature_scan` lock in the v5.1 API. The legacy `pes.graph_reaction_profile` (matplotlib via `--graph`) stays as-is for back-compat. | ✅ Done (phase 1) |
